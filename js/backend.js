...
function addDSA(idProvider) {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            $('#ds_pywrapper' + newId).trigger("change", {
                datasource: data
            });
            ...
            $('#ds_accesspoint' + newId).on("change", {
                datasource: data
            }, function (event) {
            ...
            $('#ds_accesspoint' + newId).trigger("change", {
                datasource: data
            });
            ...
            if (data.length == 0) {
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    $(listItem).on("click", {
                        ...
                        getMetadataForm(idProvider, event.data.dat)
                    });
...
function getMetadataForm(idProvider, data) {
    ...
    // trigger changes to the DSA Point
    $('#ds_accesspoint' + data["id"]).on("change", {
        datasource: data
    }, function (event) {
    ...
    // trigger changes of alternative pywrapper
    $('#ds_pywrapper' + data["id"]).on("change", {
        datasource: data
    }, function (event) {
    ...
    // trigger value of alternative pywrapper
    $('#ds_pywrapper' + data["id"]).trigger("change", {
        datasource: data
    });
    ...
    $("#collapsible_" + data["id"]).accordion({
        collapsible: true,
        active: false
    });
}
